...
export async function getStockFinancials(
  ...
  try {
    ...
    const marketCap =
      quote.marketCap || calculatedMarketCap || fallbackMarketCap;
...
export async function getStockHistory(ticker: string, period: string = '1y') {
  try {
    ...
    console.error(
      `[yahooFinance] Error fetching history for ${ticker}:`,
      error
    );
    ...
    if (
      ...
      if (errMsg.includes('Not Found') || errMsg.includes('404')) {
        throw new Error(
          `Stock symbol ${ticker} not found. Please verify the ticker symbol.`
        );
      } else if (errMsg.includes('network') || errMsg.includes('timeout')) {
        throw new Error(
          `Network error while fetching data for ${ticker}. Please try again.`
        );
      } else {
        throw new Error(
          `Failed to fetch history for ${ticker}. This stock may not have sufficient historical data available.`
        );
      }
    } else {
      throw new Error(
        `Failed to fetch history for ${ticker}. An unknown error occurred.`
      );
    }
  }
}
