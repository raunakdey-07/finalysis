...
function colorSpectrum(
  ...
  if (min !== null && max !== null) {
    ...
    percent = good
      ...
      : Math.max(0, Math.min(1, 1 - score / 10));
  }
...
export function calcPiotroski(mod: FinancialData): ScoreResult {
  try {
    ...
    if (roa0 > roa1) score++;
    // 4. CFO > ROA
    if (cf[0].totalCashFromOperatingActivities / bs[0].totalAssets > roa0)
      score++;
    // 5. Lower leverage
...
export function calcBeneish(mod: FinancialData): ScoreResult {
  try {
    ...
    const dsri =
      bs[t].netReceivables /
      ic[t].totalRevenue /
      (bs[t1].netReceivables / ic[t1].totalRevenue);
    // GMI
    const gm_t =
      (ic[t].totalRevenue - ic[t].costOfRevenue) / ic[t].totalRevenue;
    const gm_t1 =
      (ic[t1].totalRevenue - ic[t1].costOfRevenue) / ic[t1].totalRevenue;
    const gmi = gm_t1 / gm_t;
    // AQI
    const aqi =
      (1 -
        (bs[t].totalCurrentAssets + (bs[t].propertyPlantEquipmentNet || 0)) /
          bs[t].totalAssets) /
      (1 -
        (bs[t1].totalCurrentAssets + (bs[t1].propertyPlantEquipmentNet || 0)) /
          bs[t1].totalAssets);
    // SGI
    ...
    const dep_t =
      (cf[t].depreciation || 1) /
      ((bs[t].propertyPlantEquipmentNet || 1) + (cf[t].depreciation || 1));
    const dep_t1 =
      (cf[t1].depreciation || 1) /
      ((bs[t1].propertyPlantEquipmentNet || 1) + (cf[t1].depreciation || 1));
    const depi = dep_t1 / dep_t;
    // SGAI
    const sgai =
      ic[t].sellingGeneralAdministrative /
      ic[t].totalRevenue /
      (ic[t1].sellingGeneralAdministrative / ic[t1].totalRevenue);
    // LVGI
    const lvgi =
      (bs[t].totalCurrentLiabilities + (bs[t].longTermDebt || 0)) /
      bs[t].totalAssets /
      ((bs[t1].totalCurrentLiabilities + (bs[t1].longTermDebt || 0)) /
        bs[t1].totalAssets);
    // TATA
    const tata =
      (ic[t].netIncome - cf[t].totalCashFromOperatingActivities) /
      bs[t].totalAssets;
    // M-Score
    const score =
      -4.84 +
