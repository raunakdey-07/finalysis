...
export default async function handler(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[getStockDetails] Yahoo API failed. Status: ${response.status}`
      );
    ...
    if (
      ...
      console.warn(
        `[getStockDetails] No quote data found for ticker: ${ticker}`
      );
      return res
        .status(404)
        .json({ error: `No quote data found for ticker: ${ticker}` });
    }
