...
async function fetchWithTimeout(
  ...
  console.log(
    `[fetchWithTimeout] Starting fetch for: ${resource} with timeout: ${timeout}ms`
  );
  const controller = new AbortController();
  const id = setTimeout(() => {
    console.warn(
      `[fetchWithTimeout] Aborting fetch for ${resource} due to timeout.`
    );
    controller.abort();
  }, timeout);
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      console.error(
        `[fetchWithTimeout] Fetch aborted for ${resource}: Timeout`
      );
      throw new Error(
        `Request to Yahoo API timed out after ${timeout / 1000} seconds.`
      );
    }
...
export default async function handler(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[searchStocks] Yahoo API failed. Status: ${response.status}`
      );
    ...
    console.log(
      '[searchStocks] Yahoo API response (first 300 chars):',
    ...
    return res.status(isTimeoutError ? 504 : 500).json({
      error: isTimeoutError
        ? 'Yahoo API request timed out.'
        : 'Internal server error processing stock search.',
